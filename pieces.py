...
class Pawn(Pieces):
    ...
    def make_move(self):
        ...
        for i in (-1,1):
                if 0<self.col+i<9:
                    boxpiece = bo.piece_name((self.row+(1*forward),self.col+i))
                    if boxpiece!=" " and boxpiece.color!=self.color:
                        self.moves.append((self.row+(1*forward),self.col+i))
        return self.moves
    def checksides(self):
        for i in (-1, 1):
            if 0<self.col+i<9:
                boxpiece = bo.piece_name((self.row,self.col+i))
                if type(boxpiece) ==Pawn and boxpiece.color!=self.color and boxpiece.unpassant==1:
